...
class Helpers {
	 ...
	 */
	public static function download_import_files( $import_file_info, $thb_import_options ) {
		...
		if (array_key_exists('ty-contents', $thb_import_options)) {
		...
		if (array_key_exists('ty-theme-options', $thb_import_options)) {

			// ----- Set Option Tree File Path -----
			// Get theme options file as well. If defined!
			if ( ! empty( $import_file_info['import_theme_options_url'] ) ) {
				// Setup filename path to save the customizer content.
				$theme_options_filename = apply_filters( 'pt-ocdi/downloaded_theme_options_file_prefix', 'demo-theme_options-import-file_' ) . self::$demo_import_start_time . apply_filters( 'pt-ocdi/downloaded_theme_options_file_suffix_and_file_extension', '.txt' );

				// Download the customizer import file.
				$downloaded_files['theme_options'] = $downloader->download_file( $import_file_info['import_theme_options_url'], $theme_options_filename );
				// Return from this function if there was an error.
				if ( is_wp_error( $downloaded_files['theme_options'] ) ) {
					return $downloaded_files['theme_options'];
				}
			}

		}
		if (array_key_exists('ty-widgets', $thb_import_options)) {
