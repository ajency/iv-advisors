...
class OneClickDemoImport {
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( isset($_GET['page']) && 'thb-demo-import' == $_GET['page'] ) ) {
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			} else if ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.

				// Download the import files (content, widgets and customizer files).
				$import_options_str = filter_input( INPUT_POST, 'thb_import_options', FILTER_SANITIZE_STRING );
				parse_str($import_options_str, $thb_import_options);
				$this->selected_import_files = Helpers::download_import_files( $this->import_files[ $this->selected_index ], $thb_import_options );
	...
	public function setup_plugin_with_filter_data() {
		...
		$importer_options = apply_filters( 'pt-ocdi/importer_options', array(
			'fetch_attachments' => $fetch_attacments,
		) );
